...
def fetch_data():
    ...
    vacunacion_url = "https://www.datos.gov.co/resource/6i25-2hdt.csv"

    terridata = pd.read_csv(terridata_url)
    sivigila = pd.read_csv(sivigila_url)
    vacunacion = pd.read_csv(vacunacion_url)

    return terridata, sivigila, vacunacion


# Paso 2: Combinación de Datos
def combine_data(terridata, sivigila, vacunacion):
    combined_data = pd.merge(terridata, sivigila, on="municipio")
    combined_data = pd.merge(combined_data, vacunacion, on="departamento")
...
terridata, sivigila, vacunacion = fetch_data()
combined_data = combine_data(terridata, sivigila, vacunacion)
...
new_data = pd.DataFrame(
    {
        ...
        "cobertura_vacunacion": [80, 90],  # Ejemplo de datos de cobertura de vacunación
    }
)
